	...
	Describe("Node", func() {
		...
		Describe(".PUT()", func() {
			It("defines PUT request handler", func() {
				prms := params.New("/articles/11", "PUT", map[string]interface{}{})
				Expect(router.Handler(prms)).To(BeNil())
				articles := root.Child("articles", &DumbForm{})
				article := articles.Child(":article_id", &articleIDForm{})
				article.PUT(ExampleHandlerFunc, "description1")
				Expect(router.Handler(prms)).NotTo(BeNil())
				val, ok := prms.Get(":article_id")
				Expect(ok).To(BeTrue())
				Expect(val).NotTo(BeNil())
		...
		Describe(".PATCH()", func() {
			It("defines PATCH request handler", func() {
				prms := params.New("/articles/11", "PATCH", map[string]interface{}{})
				Expect(router.Handler(prms)).To(BeNil())
				articles := root.Child("articles", &DumbForm{})
				article := articles.Child(":article_id", &articleIDForm{})
				article.PATCH(ExampleHandlerFunc, "description1")
				Expect(router.Handler(prms)).NotTo(BeNil())
				val, ok := prms.Get(":article_id")
				Expect(ok).To(BeTrue())
				Expect(val).NotTo(BeNil())
		...
		Describe(".DELETE()", func() {
			It("defines DELETE request handler", func() {
				prms := params.New("/articles/11", "DELETE", map[string]interface{}{})
				Expect(router.Handler(prms)).To(BeNil())
				articles := root.Child("articles", &DumbForm{})
				article := articles.Child(":article_id", &articleIDForm{})
				article.DELETE(ExampleHandlerFunc, "description1")
				Expect(router.Handler(prms)).NotTo(BeNil())
				val, ok := prms.Get(":article_id")
				Expect(ok).To(BeTrue())
				Expect(val).NotTo(BeNil())
